...
| 1.4.0 | 2026-05-17 | Prompt-engineering audit pass mirroring the Python project's same-day audit. Sharpened confidence rubrics in all 11 module skills (per-tier-with-concrete-conditions); added ambiguity rules to `arr-module-04-corporate` (in-flight acquisitions, mergers of equals, spin-offs); loosened `arr-module-09-competitors` from "Exactly 3" to "1–3 truly direct, no padding" (`minItems: 3 → 1`); added input-trust boundary note to the orchestrator's subagent prompt template (prompt-injection defense for WebSearch / WebFetch content). |
